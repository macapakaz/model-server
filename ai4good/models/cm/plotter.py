...
def population_format(num, dp=0):
    ...
    while abs(num) >= 1000:
        magnitude += 1
        num /= 1000.0
    # unless force to be dp=1... return 0 dp if (K or M or B and number >10)
    if dp == 0 and not (num/100 < 1 and magnitude >= 1):
...
def figure_generator(sols, params, cats_to_plot):
    ...
    for sol in sols:
        for name in categories.keys():
            if name in cats_to_plot:
                y_plot = np.asarray(
                    100*sol['y_plot'][categories[name]['index']])
    ...
    pop_vec_log_intermediate = np.linspace(log_range[0], ceil(
        np.log10(pop_vec_lin[-1])), 1 + ceil(np.log10(pop_vec_lin[-1])-log_range[0]))
    ...
    layout = go.Layout(
        ...
        yaxis2=dict(
            ...
            range=yax['range'],
            side='right',
            ticktext=[population_format(0.01*vec[i])
                      for i in range(len(pop_vec_lin))],
...
def uncertainty_plot(sols, params, cats_to_plot, confidence_range=None):
    ...
    labels = ['1', '75-97.5 percentile',
              '25-75 percentile', '2.5-25 percentile']
        ...
        for name in categories.keys():
            if name == cats_to_plot:
                ...
                for i in range(population_frame.shape[0]):  # age_categories
                    y_plot = 100*sol['y'][categories[name]
                                          ['index'] + i*params.number_compartments, :]

                    # first one says e.g. infected
                    legend_name = categories[name]['longname'] + \
                        ': ' + np.asarray(population_frame.Age)[i]
