...
from ai4good.models.nm.network_model import NetworkModel
...
def run_model(_model: str, _profile: str, camp: str, load_from_cache: bool, save_to_cache: bool,
    ...
    if is_save_plots or is_show_plots:
        save_plots(_model, mr, res_id, is_save_plots, is_show_plots)
    if is_save_report:
        save_report(mr, res_id)
    return mr


def save_plots(_model, mr, res_id, is_save_plots, is_show_plots):
    if _model == CompartmentalModel.ID:
        ...
        sol = mr.get('standard_sol') #TODO: add some charts if needed
    else:
        raise RuntimeError("Unknown model: "+_model)
